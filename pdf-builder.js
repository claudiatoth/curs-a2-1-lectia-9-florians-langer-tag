// PDF BUILDER — Florians langer Tag (A2.1 · Lecția 9)
(function () {
    ...
    function buildCast() {
        return `<div class="cast-banner">
            ...
            <div class="cast-grid">
                <div class="cast-card"><img src="images/florian.png" alt="Florian"><div class="name">Florian</div><div class="detail">Doctor · o zi lungă</div></div>
                <div class="cast-card"><img src="images/carolina.png" alt="Carolina"><div class="name">Carolina</div><div class="detail">Prietena · îl ascultă</div></div>
                <div class="cast-card"><img src="images/andreea.png" alt="Andreea"><div class="name">Andreea 🇷🇴</div><div class="detail">Studentă · Potsdam</div></div>
    ...
    function buildTheory() {
        ...
        t = t.replace(/<div class="theory-box"\s+style="background:\s*#F5F0E8[^"]*"[^>]*>/g, '<div class="theory-box warn-box">');
        return `<h1 class="chapter">📘 1. Teorie — Florians langer Tag: Reflexive Verben (mich vs mir + Perfekt)</h1>` + t;
    }
    ...
    function fillPromptBlock(title, instruction, data) {
        ...
        data.forEach((it, i) => {
            ...
            h += `<div class="ex-item"><span class="ex-num">${i + 1}</span><div class="ex-body"><div class="ex-q">${filled}</div>${it.hint ? `<div class="ex-explanation">💡 ${it.hint}</div>` : ''}</div></div>`;
        });
        return h + `</div>`;
    }
    ...
    function buildExercises() {
        ...
        if (typeof ex1Data !== 'undefined') {
            html += fillPromptBlock('Übung 1 — Reflexiv Akkusativ', 'mich / dich / sich / uns / euch / sich — cazul normal (fără obiect).', ex1Data);
        }
        if (typeof ex2Data !== 'undefined') {
            html += choiceBlock('Übung 2 — Akkusativ sau Dativ? (mich/mir)', 'Obiect direct (die Hände, die Zähne, die Jacke) → Dativ (mir/dir). Altfel → Akkusativ (mich/dich).', ex2Data);
        }
        if (typeof ex3Data !== 'undefined') {
            html += fillPromptBlock('Übung 3 — Reflexiv Dativ (mir/dir + obiect)', 'Toate au un obiect direct → Dativ: ich→mir, du→dir, restul ca la Akkusativ.', ex3Data);
        }
        if (typeof ex4Data !== 'undefined') {
            html += fillPromptBlock('Übung 4 — Reflexive la Perfekt (Partizip II)', 'haben + pronume reflexiv + Partizip II. be-/ver-/ent- → fără ge- · trennbar → -ge- la mijloc.', ex4Data);
        }
        if (typeof ex5Data !== 'undefined') {
            html += translateBlock('Übung 5 — Traducere RO → DE', 'Atenție la mich vs mir (apare un obiect?) și la Perfekt cu haben.', ex5Data);
        }
        return html;
    }
    ...
    function buildVerbs() {
        let html = `<h1 class="chapter new-section">🔁 4. Verbele reflexive — Präsens · Präteritum · Perfekt</h1>
            <div class="andreea-note">
                <div class="andreea-note-content">
                    <div class="speaker">📌 Andreea îți spune:</div>
                    <div>Pronumele reflexiv se conjugă odată cu verbul (mich/dich/sich...) și rămâne și la Perfekt. Cu un obiect direct → Dativ (mir/dir). Prefixele be-/ver-/ent- NU primesc ge-. Toate fac Perfekt cu HABEN.</div>
                </div>
            </div>`;
